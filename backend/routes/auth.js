...
// --- SIGNUP ROUTE (UPDATED) ---
// @route   POST api/auth/signup
// @desc    Register a new user and save their location and phone if they are a worker
router.post('/signup', async (req, res) => {
  console.log('--- New Signup Request Received ---');
  console.log('Request Body:', req.body);

  // MODIFIED: Added 'phone' to the destructured properties
  ...
  try {
    ...
    // MODIFIED: Update workerDetails creation to include the phone number
    if (role === 'worker') {
      userData.workerDetails = {}; // Initialize workerDetails object
      
      // Add phone if it exists
      if (phone) {
        userData.workerDetails.phone = phone;
      }
