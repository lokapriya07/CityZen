...
async function geocodeAddress(address) {
    ...
    try {
        ...
        // Log the response from Google to help debug
        ...
        if (response.data.status === "OK" && response.data.results.length > 0) {
            const loc = response.data.results[0].geometry.location;
            console.log(`Geocoding success for '${address}':`, [loc.lng, loc.lat]);
            return [loc.lng, loc.lat]; // GeoJSON format
        }
    } catch (error) {
        console.error("Geocoding failed:", error.message);
    ...
    async (req, res) => {
        try {
            ...
            if (req.body.gpsCoordinates?.lat && req.body.gpsCoordinates?.lng) {
                ...
                // =================================================================
                // ✅ START: FIX
                // We now combine address, city, and pincode to get an accurate location.
                // =================================================================
                const fullAddress = `${req.body.address}, ${req.body.city}, ${req.body.pincode}`;
                console.log("Attempting to geocode full address:", fullAddress); // Good for debugging
    ...
    async (req, res) => {
        try {
            ...
            if (report.userSharedLocation) {
                ...
                const a = 
                    Math.sin(dLat/2) * Math.sin(dLat/2) +
                    Math.cos(userLat * Math.PI / 180) * Math.cos(workerLat * Math.PI / 180) * Math.sin(dLng/2) * Math.sin(dLng/2);
                const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a));
                distance = R * c; // Distance in km
            }
