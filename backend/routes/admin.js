...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      await report.save()
console.log(`--- DEBUG: Creating task from report. Urgency is: ${report.urgency} ---`);
      // ✅ THIS IS THE CORRECTED DYNAMIC TASK CREATION
      const task = new Task({
        report: report._id,
        assignedWorker: workerId,
        assignedBy: req.user.id,
        ...
        description: report.description,
        priority: report.urgency, // Directly use the report's urgency
        status: "assigned",
        location: report.location,
      })

      await task.save()
console.log(`--- DEBUG: New task saved to database with priority: ${task.priority} ---`);
      // 🔥 Update analytics
      await updateAnalytics({ reportsReceived: 1, tasksAssigned: 1 })
