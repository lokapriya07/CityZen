  ...
  return (
    <div className="space-y-6">
          ...
          <Card className="w-full max-w-md bg-white">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                <Button variant="outline" className="flex-1" onClick={() => setShowNewCampaignModal(false)} disabled={isSubmitting}>Cancel</Button>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <select value={rewardData.selectedUser} onChange={(e) => setRewardData({ ...rewardData, selectedUser: e.target.value })} className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                  ...
                  {leaderboard.map((reporter) => (<option key={reporter.id} value={reporter.id}>{reporter.name} ({reporter.points} pts)</option>))}
                </select>
              </div>
              {/* ... other form fields ... */}
              <div>
              ...
              <div className="flex gap-3 pt-4">
                <Button variant="outline" className="flex-1" onClick={() => setShowRewardModal(false)} disabled={isSubmitting}>Cancel</Button>
          ...
          <Card className="w-full max-w-2xl bg-white max-h-[90vh] flex flex-col">
            ...
            <CardContent className="flex-1 overflow-y-auto space-y-4 p-6">
              <div className="flex items-center p-2 border rounded-md">
                <input type="checkbox" id="selectAll" checked={selectedUsers.length === leaderboard.length && leaderboard.length > 0} onChange={selectAllUsers} className="w-4 h-4 mr-3" />
              ...
              <div className="space-y-2 border rounded-md p-2 max-h-60 overflow-y-auto">
                {leaderboard.map((reporter) => (
                  <div key={reporter.id} className="flex items-center space-x-3 p-2 rounded hover:bg-gray-50 cursor-pointer" onClick={() => toggleUserSelection(reporter.id)}>
                    ...
                    <Avatar className="w-8 h-8"><AvatarImage src={reporter.avatar || "/placeholder.svg"} /><AvatarFallback>{reporter.name.split(" ").map(n => n[0]).join("")}</AvatarFallback></Avatar>
                    <div className="flex-1"><p className="text-sm font-medium">{reporter.name}</p><p className="text-xs text-gray-500">{reporter.points} pts • {reporter.reportCount} reports</p></div>
                  </div>
                ))}
              </div>
              {/* ... other form fields ... */}
            ...
            <div className="flex gap-3 p-6 border-t">
              <Button variant="outline" className="flex-1" onClick={() => setShowBulkRewardModal(false)} disabled={isSubmitting}>Cancel</Button>
              <Button className="flex-1" onClick={handleBulkReward} disabled={selectedUsers.length === 0 || isSubmitting}>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <Card className="w-full max-w-md bg-white">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                <Button variant="outline" className="flex-1" onClick={() => setShowAutomatedRulesModal(false)} disabled={isSubmitting}>Cancel</Button>
      ...
      <Tabs defaultValue="leaderboard" className="space-y-6">
        ...
        {/* --- Leaderboards Tab (DYNAMIC) --- */}
        <TabsContent value="leaderboard" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center justify-between"><span>Top Reporters</span><Badge className="bg-green-100 text-green-700 border-green-300">
                  This Month
                </Badge></CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {leaderboard.length > 0 ? leaderboard.map((reporter, index) => (
                    <div key={reporter.id} className="flex items-center space-x-4 p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center space-x-3">
                        ...
                        <Avatar className="w-10 h-10"><AvatarImage src={reporter.avatar || "/placeholder.svg"} /><AvatarFallback>{reporter.name.split(" ").map(n => n[0]).join("")}</AvatarFallback></Avatar>
                      </div>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-gray-500">{reporter.reportCount} reports</p>
                      </div>
                    </div>
                  )) : (
                    <p className="text-center text-gray-500">No reporter data available.</p>
                  )}
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center justify-between"><span>Most Active Areas</span><Badge className="bg-green-100 text-green-700 border-green-300">
                  Engagement Rate
                </Badge></CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {activeAreas.length > 0 ? activeAreas.map((area, index) => (
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500">{area.reportCount} reports • {area.residentCount} residents</p>
                          </div>
                        </div>
                    ...
                    <p className="text-center text-gray-500">No area data available.</p>
                  )}
                </div>
              </CardContent>
            </Card>
