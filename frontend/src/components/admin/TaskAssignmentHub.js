...
const fetchApi = async (url, method = 'GET', body = null) => {
    ...
    // 1. Get the token from browser storage instead of hardcoding it.
    //    Make sure you save the token with the key 'authToken' after login.
    ...
    if (!response.ok) {
        // 3. (Optional but Recommended) If the token is invalid/expired, automatically redirect to login.
