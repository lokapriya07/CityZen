...
//                       <Badge variant={item.avgTime <= item.target ? "secondary" : "destructive"} className="text-xs">
...
export function AnalyticsDashboard({ reports = [], workers = [] }) {
  ...
  const mockReports =
    reports.length > 0
      ? reports
      : [
          { id: "1", status: "resolved", reportedAt: "2024-01-15T08:30:00Z", resolvedAt: "2024-01-15T10:45:00Z" },
...
<CardContent>
  <div className="space-y-3">
    {repeatZones.map((zone) => (
      <div
        key={zone.zone}
        className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 border rounded-lg gap-3 sm:gap-2 w-full"
      >
        {/* Zone Info */}
        ...
        {/* Badge + Button */}
        <div className="flex flex-col xs:flex-row items-center justify-center sm:justify-end gap-2 w-full sm:w-auto">
          <Badge
            variant={
              zone.trend === "increasing"
                ? "destructive"
            ...
            className="text-xs sm:text-sm px-2 py-1 w-full xs:w-auto text-center"
          >
            {zone.trend}
          </Badge>
          <Button
            variant="outline"
            size="sm"
            className="h-7 text-xs sm:text-sm bg-transparent w-full xs:w-auto"
          >
            Investigate
          </Button>
        </div>
