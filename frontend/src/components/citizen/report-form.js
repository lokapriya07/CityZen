...
// import './UploadImage.css'; // You might want to rename this CSS file to ReportForm.css for consistency
...
//         description: '', urgency: '', photos: null,
//         previousReports: '', preferredContact: '',
//         updates: false,
//     };
...
//     useEffect(() => {
//         const requiredFields = ['fullName', 'email', 'phone', 'address', 'city', 'pincode', 'state', 'wasteType', 'description'];
//         const filledCount = requiredFields.filter(field => formData[field] && formData[field].trim() !== '').length;
...
//         try {
//             const res = await axios.post("http://localhost:8000/upload-image", uploadFormData, {
...
//             // Success! Show preview and set data
//             setClassificationResult(resultMessage);
//             setFormData(prev => ({ ...prev, photos: fileToValidate })); // Store the single valid file
...
//             }
//         }
//         // Append the validated photo file
//         submissionData.append('image', formData.photos);

//         setShowSuccess(true); // To disable button and show loading state
        
//         try {
//             // NOTE: Replace with your actual report submission endpoint
//             const response = await axios.post('http://localhost:8001/api/reports', submissionData, {
//                 headers: {
//                     'Content-Type': 'multipart/form-data',
//                      // If you use authentication, add the token
...
//             setShowSuccess(false); // Re-enable button on failure
//         }
//     };
...
//                             <label htmlFor="photos">Upload Photo (Must be a valid garbage image)</label>
//                              {/* Conditionally render the image preview only when it exists */}
//                             {imagePreview && (
...
//                             {showSuccess ? 'Submitting...' : 'Submit Complaint'}
//                         </button>
...
import React, { useState, useEffect, useRef } from 'react';
import axios from 'axios';
import './UploadImage.css'; // Make sure you have this CSS file or rename it
...
export function ReportForm() {
    ...
    const handleFileChangeAndValidation = async (event) => {
        ...
        try {
            // This URL should point to your AI classification service
            const res = await axios.post("http://localhost:8000/upload-image", uploadFormData, {
                headers: { "Content-Type": "multipart/form-data" },
            });
            const resultMessage = res.data.message || "";
    ...
    const handleSubmit = async (e) => {
        ...
        // Since image classification is a required step, we check for the photo.
        if (!formData.photos) {
            alert("Please upload and validate a photo of the waste spot before submitting.");
            return;
        }
        ...
        // Append all fields from the form state to the FormData object.
        for (const key in formData) {
            if (key === 'photos' && formData.photos) {
                // The backend route expects the file(s) under the key 'images'.
                submissionData.append('images', formData.photos);
        ...
        setShowSuccess(true); // Show loading/submitting state
    
        try {
            const token = localStorage.getItem('token'); // Get the user's token from localStorage
            if (!token) {
                alert("Authentication error: You are not logged in. Please log in to submit a report.");
                setShowSuccess(false);
                return;
            }
    
            // Make the API call to your backend report submission endpoint.
            const response = await axios.post('http://localhost:8001/api/reports', submissionData, {
                headers: {
            ...
            if (response.status === 201) {
                // The success message will be shown via the `showSuccess` state.
                ...
                throw new Error(response.data.message || 'Submission failed with an unexpected status.');
            }
    
        } catch (error) {
            console.error("Submission Error:", error);
            
            // Provide specific feedback to the user based on the error response.
            if (error.response && error.response.data && error.response.data.errors) {
                ...
                alert("There was an error submitting your report. Please check the console for details.");
            }
            
            setShowSuccess(false); // Re-enable the submit button on failure
        }
    };
    ...
    const indianStates = [
        ...
        "Nagaland", "Odisha", "Punjab", "Rajasthan", "Sikkim", "Tamil Nadu", "Telangana", "Tripura", "Uttar Pradesh", "Uttarakhand", "West Bengal", "Delhi"
    ];
