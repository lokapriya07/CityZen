...
export function EnhancedProgressTracker({ steps, estimatedDate }) {
  // Calculate total completed (but don't overshoot)
  const totalSteps = steps.length;
  const completedSteps = steps.filter((s) => s.completed).length;

  // If all steps done → cap width at 100% minus half-circle spacing (so line stops neatly)
  const progressWidth =
    completedSteps === totalSteps
      ? "100%"
      : `${((completedSteps - 1) / (totalSteps - 1)) * 100}%`;
      ...
      {estimatedDate && (
        <div className="text-center mb-8">
          <p className="text-sm text-gray-600 font-medium">
            ...
            <span className="text-gray-900">{estimatedDate}</span>
          </p>
        </div>
      )}
          ...
          {steps.map((step, index) => (
            ...
            >
              ...
              <div className="mt-4 space-y-1 max-w-[100px]">
                <h4
                  className={`text-sm font-semibold leading-tight ${step.completed
                      ? "text-gray-900"
                      : step.current
                        ? "text-gray-700"
                        : "text-gray-500"
